...
menucolors(void)
{
	...
	back = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DWhite);
	high = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DBlack);	/* dark green */
	bord = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DBlack);	/* not as dark green */
...
int
nmenuhit(int but, Mousectl *mc, Menu *menu, Screen *scr)
{
